...
import club.auth.hydraware.util.Timer;
...
public class AutoArmor extends Module {
    ...
    @Override
    public void update() {
        if (nullCheck() || AutoArmor.mc.currentScreen instanceof GuiContainer && !(AutoArmor.mc.currentScreen instanceof GuiInventory)) {
            return;
        }
        ...
        if (this.timer.passedMs((int) ((float) this.delay.getValue() * HydraWare.serverManager.getTpsFactor()))) {
            if (!this.taskList.isEmpty()) {
